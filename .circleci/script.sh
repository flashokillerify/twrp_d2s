...
# Clone device tree
git clone "$DEVICE_TREE" --depth=1 device/"$VENDOR"/"$CODENAME"/
...
DATE=$(date +"%Y%m%d")
	...
	if [ -f "$IMG" ]; then
		TWRP_IMGAGE="twrp-3.6.0_11.0-"$CODENAME"_"$DATE".img"
		mkdir sender
